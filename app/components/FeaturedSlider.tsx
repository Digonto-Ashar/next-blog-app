    ...
    <section className="relative w-full h-[800px] -mt-24">
      ...
      >
        {posts.map((post) => {
          ...
          const excerpt =
            post.content.root.children[0]?.children[0]?.text.slice(0, 250) +
            "...";
          ...
          return (
            <SwiperSlide key={post.id} className="relative bg-black">
              ...
              <div className="relative z-20 h-full flex flex-col justify-end text-left text-white p-8 md:p-16">
                {post.categories && post.categories.id ? (
                  <Link
                    href={`/categories/${post.categories.id}`}
                    onClick={(e) => e.stopPropagation()}
                    className="bg-white/10 backdrop-blur-sm rounded-full px-4 py-1 text-sm mb-3 w-fit transition-colors hover:bg-white/20"
                  >
